...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public static Person GetPersonFromInputParameter( object input, ILavaRenderContext context )
        {
            ...
            {
                var inputAsString = s.ToStringSafe().Trim().ToLower();
                if ( inputAsString == "currentperson" )
                {
                    return GetCurrentPerson( context );
                }
                else if ( inputAsString == "currentvisitor" )
                {
                    var personAlias = GetCurrentVisitorInContext( context );
