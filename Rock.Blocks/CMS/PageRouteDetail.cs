...
namespace Rock.Blocks.CMS
{
    ...
    [Rock.SystemGuid.EntityTypeGuid( "d167e604-faf5-4a22-8569-faad11e0eb8b" )]
    [Rock.SystemGuid.BlockTypeGuid( "d8048391-a53d-4649-8d69-fd3d0bc20937" )]
    public class PageRouteDetail : RockObsidianDetailBlockType
    {
        ...
        private bool ValidatePageRoute( PageRoute pageRoute, RockContext rockContext, out string errorMessage )
        {
            ...
            var pageCache = Web.Cache.PageCache.Get( pageRoute.Page.Id );
        ...
        /// <param name="rockContext">The rock context.</param>
        private void SetBoxInitialEntityState( DetailBlockBox<PageRouteBag, PageRouteDetailOptionsBag> box, bool loadAttributes, RockContext rockContext )
        {
        ...
        /// <param name="loadAttributes"><c>true</c> if attributes and values should be loaded; otherwise <c>false</c>.</param>
        /// <returns>A <see cref="PageRouteBag"/> that represents the entity.</returns>
        private PageRouteBag GetEntityBagForView( PageRoute entity, bool loadAttributes )
        {
            if ( entity == null )
            {
                return null;
            ...
            bag.Site = GetPageSite();

            return bag;
        }

        private string GetPageSite()
        {
            var pageId = RequestContext.GetPageParameter( PageParameterKey.PageRouteId ).AsIntegerOrNull();
            if ( pageId.HasValue )
            {
                var page = Rock.Web.Cache.PageCache.Get( pageId.Value );
        ...
        /// <param name="loadAttributes"><c>true</c> if attributes and values should be loaded; otherwise <c>false</c>.</param>
        /// <returns>A <see cref="PageRouteBag"/> that represents the entity.</returns>
        private PageRouteBag GetEntityBagForEdit( PageRoute entity, bool loadAttributes )
        {
            if ( entity == null )
            {
                return null;
        ...
        public BlockActionResult Save( DetailBlockBox<PageRouteBag, PageRouteDetailOptionsBag> box )
        {
            using ( var rockContext = new RockContext() )
            {
                ...
                if ( !entity.IsValid )
                {
                    return ActionBadRequest( entity.ValidationResults.FirstOrDefault().ErrorMessage );
                }
